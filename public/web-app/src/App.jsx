...
import { Wifi, Thermometer, Droplets, Wind, Cloud, Mountain, Gauge, Globe,  } from 'lucide-react';
import "./App.css";
...
// --- ✨ MAJOR UPDATE: Revamped Web API feature to show all pollutant details ---

// --- API Configuration ---
const OWM_API_KEY = "51315e05d0207ef35f380db52504a2d9"; 
const LATITUDE = 21.7599; // Bhavnagar Latitude
const LONGITUDE = 72.1327; // Bhavnagar Longitude
const API_URL = `https://api.openweathermap.org/data/2.5/air_pollution?lat=${LATITUDE}&lon=${LONGITUDE}&appid=${OWM_API_KEY}`;

// Helper function to interpret OpenWeatherMap's AQI scale (1-5)
const getWebAqiInfo = (aqi) => {
    switch (aqi) {
        case 1: return { label: 'Good', color: 'text-green-400', bg: 'bg-green-500/20' };
        case 2: return { label: 'Fair', color: 'text-lime-400', bg: 'bg-lime-500/20' };
        case 3: return { label: 'Moderate', color: 'text-yellow-400', bg: 'bg-yellow-500/20' };
        case 4: return { label: 'Poor', color: 'text-orange-400', bg: 'bg-orange-500/20' };
        case 5: return { label: 'Very Poor', color: 'text-red-500', bg: 'bg-red-500/20' };
        default: return { label: 'Loading...', color: 'text-gray-400', bg: 'bg-gray-700' };
    }
};

/**
 * PollutantCard: A small, reusable card for displaying a single pollutant.
 */
const PollutantCard = ({ name, value, unit }) => {
    return (
...
/**
 * WebApiAqiDetails: A new, detailed component to display all AQI data from OpenWeatherMap.
 */
const WebApiAqiDetails = () => {
    ...
    useEffect(() => {
        ...
        fetchApiData();
        const intervalId = setInterval(fetchApiData, 5000); // Refresh every 30 mins
        return () => clearInterval(intervalId);
    }, []);

    const aqiValue = apiData?.main.aqi;
    const components = apiData?.components || {};
    const aqiInfo = getWebAqiInfo(aqiValue);

    if (status !== 'Live' && !apiData) {
        return (
             <div className="bg-slate-800/60 p-6 rounded-xl text-center">
                <p className="text-gray-400">{status} Web API Data...</p>
            </div>
        )
    }

    return (
        <div className={`p-6 rounded-2xl shadow-2xl ${aqiInfo.bg}`}>
            <div className="flex justify-between items-center mb-4">
                <div>
                    <h3 className="text-lg font-medium text-gray-300">
                        <Globe size={18} className="inline-block mr-2" />
                        Bhavnagar - Web Air Quality Index
                    </h3>
                    <p className="text-sm text-gray-400">Data provided by OpenWeatherMap</p>
                </div>
                <div className="text-right">
                    <p className={`text-5xl font-extrabold ${aqiInfo.color}`}>{aqiValue}</p>
                    <p className={`text-xl font-semibold ${aqiInfo.color}`}>{aqiInfo.label}</p>
                </div>
            </div>
            
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-8 gap-3 mt-4 pt-4 border-t border-slate-700">
                <PollutantCard name="PM2.5" value={components.pm2_5} unit="µg/m³" />
                <PollutantCard name="PM10" value={components.pm10} unit="µg/m³" />
                <PollutantCard name="Ozone" value={components.o3} unit="µg/m³" />
                <PollutantCard name="NO₂" value={components.no2} unit="µg/m³" />
                <PollutantCard name="SO₂" value={components.so2} unit="µg/m³" />
                <PollutantCard name="CO" value={components.co} unit="µg/m³" />
                <PollutantCard name="NH₃" value={components.nh3} unit="µg/m³" />
                <PollutantCard name="NO" value={components.no} unit="µg/m³" />
            </div>
        </div>
    );
};
...
// --- Existing Helper Functions & Components (Unchanged) ---
...
const DataChart = ({ data }) => {
    return (
        <div className="bg-slate-800/60 p-6 rounded-xl shadow-lg mt-8">
            <h3 className="text-xl font-semibold text-white mb-4">📊 IoT Sensor - Historical Trends</h3>
...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onValue(sensorDataRef, (snapshot) => {
            if (snapshot.exists()) {
                ...
                setSensorData(data);
                setConnectionStatus({ text: 'Live (IoT Sensor)', color: 'bg-green-500/20 text-green-400' });
    ...
    return (
        <div className="space-y-8">
            {/* ✨ MODIFIED: Header is now cleaner, Web API badge has been removed. */}
            ...
            {/* ✨ NEW: Added the detailed Web API component here for better visibility */}
            <WebApiAqiDetails />

            {/* Main AQI Display (Unchanged) */}
            <div className={`p-8 rounded-2xl shadow-2xl text-center ${aqiInfo.bg}`}>
                <h3 className="text-lg font-medium text-gray-300">PM2.5 Air Quality Index (Local IoT Sensor)</h3>
            ...
            {/* AQI Info (Unchanged) */}
            <div className="bg-slate-800/60 p-6 rounded-xl shadow-lg mt-8">
                <h3 className="text-xl font-semibold text-white mb-4">ℹ️ PM2.5 AQI Levels Explained (Indian Standards)</h3>
